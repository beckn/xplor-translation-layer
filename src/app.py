...
app = FastAPI(
    ...
    version="v0.0.1",
    contact={"name": "WITSLAB"},
    docs_url="/docs",
    redoc_url="/redocs",
    openapi_url="/api/v1/openapi.json",
    openapi_tags=[
        {"name": "Healthcheck", "description": "Healthcheck operations"},
...
def translate(request: TranslationRequest):
    ...
    translated_text = translate_input(
        request.text, request.from_ln, request.to_ln, request.excluded_keys
    )
    if isinstance(translated_text, str) and translated_text.startswith(
        "Required translation package could not be installed"
    ):
        raise HTTPException(
            status_code=500, detail="Translation package installation failed."
        )
    return {
        "translated_text": translated_text,
        "target_language": request.to_ln,
